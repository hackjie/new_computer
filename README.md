...
### 📦 包含的工具
- **包管理器**: Homebrew (macOS包管理器，使用 [清华 TUNA 镜像](https://mirrors.tuna.tsinghua.edu.cn/help/homebrew/)全自动安装，阿里巴巴源加速 update/bottle)
- **终端工具**: iTerm2 (高级终端应用)
...
### 🤖 智能特性
- 🇨🇳 **国内加速**: Homebrew 使用清华 TUNA 镜像全自动安装，安装后配置阿里 `brew` / `homebrew-core` / `homebrew-cask` Git 远程与 bottle/API 环境变量，加速 `brew update` 与软件包下载
...
A: 此脚本已改用 [清华 TUNA 镜像](https://mirrors.tuna.tsinghua.edu.cn/help/homebrew/)安装，完全无交互、无 Gitee 排队。如旧脚本仍出现该提示，请拉取最新版本后重新运行。
